...
/** Read STATE.json from disk. Sanitizes then validates. Throws if file doesn't exist. */
...
/** Common typos / near-miss status values → canonical form */
...
/**
 * Best-effort sanitization of a raw STATE.json object.
...
export function sanitize(state: State): string[] {
  ...
  // ── Status auto-correct ──
  if (state.status && !VALID_STATUSES.has(state.status)) {
    const corrected = STATUS_TYPO_MAP[state.status.toLowerCase()];
    if (corrected) {
      warnings.push(
        `⚠️  Auto-corrected status: "${state.status}" → "${corrected}"`,
      );
      state.status = corrected;
    }
    // If not in typo map, let validate() throw — it's truly unknown
  }

  // ── Reason cleanup ──
  // reason is only meaningful when status === "failing"
  if (state.status !== "failing" && state.reason !== null) {
    warnings.push(
      `⚠️  Cleared reason "${state.reason}" (only valid when status is "failing")`,
    );
    state.reason = null;
  }
  // If failing but reason is not a valid enum, clear it
  if (
    state.status === "failing" &&
    state.reason !== null &&
    !VALID_REASONS.has(state.reason)
  ) {
    warnings.push(
      `⚠️  Cleared invalid reason: "${state.reason.slice(0, 80)}${state.reason.length > 80 ? "..." : ""}"`,
    );
    state.reason = null;
  }
